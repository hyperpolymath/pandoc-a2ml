...
# setup.sh — Universal setup script for pandoc-a2ml
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/pandoc-a2ml/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
main() {
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/pandoc-a2ml.git"
        info "Then: cd pandoc-a2ml && ./setup.sh"
        exit 1
    fi
